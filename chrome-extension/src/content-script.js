// overlay-shell 录像 — isolated-world content script (document_start, every page). It does NOT fetch
...
(function () {
  ...
  // ── 统一捕获 · 通用 DOM 元素解析 ─────────────────────────────────────
  // 持续上报"光标下元素"给 overlay-shell(经 SW),让 overlay-shell 不靠埋点也能知道任意网页上你指的是哪个元素。
  ...
  // 点查询(overlay-shell 给视口坐标 → elementFromPoint → 应答): 供冻屏截图按选区取元素。
  ...
  chrome.runtime.onMessage.addListener(function (msg, sender, reply) {
    if (msg.type === "REC_START") {
      ensureInjected().then(function () {
        window.postMessage({ __overlayShell: true, type: "REC_START", sid: msg.sid }, "*");
        reply({ ok: true });
      });
      return true; // async reply
    }
    if (msg.type === "REC_STOP") {
      window.postMessage({ __overlayShell: true, type: "REC_STOP" }, "*");
      reply({ ok: true });
    }
  });
})();
